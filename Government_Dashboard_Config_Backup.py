...
async def main_async(api_key, operations, endpoints, tag, template_only=False, template_name=None):
    ...
    # Initialize AsyncDashboardAPI with the government API base URL
    print("Initializing AsyncDashboardAPI for government endpoint...")
    
    # Must explicitly set base_url for AsyncDashboardAPI
...
def estimate_backup(api_key, org_id, filter_tag, template_only=False, template_name=None):
    try:
        print("Testing connection and estimating backup time...")
        ...
        test_url = "https://api.gov-meraki.com/api/v1/organizations/{org_id}"
        test_response = requests.get(test_url.format(org_id=org_id), headers=spec_headers)
        
        if test_response.status_code != 200:
            print(f"Connection test failed: {test_response.status_code}")
            print(f"Response: {test_response.text[:200]}...")
            sys.exit("API connection test failed. Please check your API key and organization ID.")
        else:
            org_data = test_response.json()
            print(f"Successfully connected to organization: {org_data.get('name')}")
        
        # Initialize DashboardAPI (using global base_url setting from meraki.config)
        ...
        sys.exit('An unexpected error occurred during estimate.')


# Modify run_backup to account for template-only backups
def run_backup(api_key, org_id, filter_tag, template_only=False, template_name=None):
    ...
    # Test connection first
    print("Testing connection to the Meraki Government API...")
    test_url = "https://api.gov-meraki.com/api/v1/organizations/{org_id}"
    test_response = requests.get(test_url.format(org_id=org_id), headers=spec_headers)
    ...
    spec_url = "https://api.gov-meraki.com/api/v1/openapiSpec"
    spec_response = requests.get(spec_url, headers=spec_headers)
    
    if spec_response.status_code != 200:
        print(f"Failed to get OpenAPI spec: {spec_response.status_code}")
        print(f"Response: {spec_response.text[:200]}...")
        sys.exit("Failed to fetch OpenAPI specification.")
...
READ_ME = '''
Backup Meraki organization configuration
----------------------------------------
This script creates a local backup of a Meraki organization's configuration settings, by making API calls
to download settings for organization, networks, devices, and templates.

CLI usage:
  python backup.py -k <api_key> -o <org_id> [-t <tag>] [-y] [-e <template_name>] [-a]
...
The output folder will contain the entire organizational hierarchy in JSON and/or YAML format.
'''
...
def main(inputs):
    if len(inputs) == 0:
        ...
        print(f"Using Meraki Government API endpoint: https://api.gov-meraki.com/api/v1")
